// fanalyzer_decision_tree.ino
// Classes: 0=normal  1=blocked  2=unbalanced  3=off
//
// FOLDER SETUP:
//   fanalyzer_decision_tree/
//   ├── fanalyzer_decision_tree.ino   <- this file
//   └── dt_model.h                    <- copy from Python output
...
void printResult(Features f, int pred) {
  Serial.println("\n--------------------------------");
  Serial.println("  MODEL: Decision Tree");
...
void loop() {
  ...
  // NOTE: Open dt_model.h and check what function/class it uses.
  //
  ...
  //int pred = dt_predict(f.dom_freq, f.max_amp, f.mean_energy);
